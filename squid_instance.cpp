...
Squid_Instance::Squid_Instance()
{
    ...
    memset(&params, 0, sizeof(Squid_Params));
    memset(&data, 0, sizeof(Squid_Data));
    memset(wifi_mac, 0, 6);
...
void Squid_Instance::begin(Squid_Network *n, Squid_Params p)
{
    network = n;
    params = p;
    data.base_alt_m = alt;
...
void Squid_Instance::setCenterLatLon(double lat, double lon)
{
    data.base_latitude = data.latitude_d = lat;
    data.base_longitude = data.longitude_d = lon;
...
void Squid_Instance::beginPath()
{
    setMode(SD_MODE_PATH);
    pathMode = SD_PATH_MODE_IDLE;
}

void Squid_Instance::endPath()
{
    setMode(SD_MODE_IDLE);
    pathMode = SD_PATH_MODE_IDLE;
}
...
void Squid_Instance::followPath(Squid_Path *p, int size)
{
    if (size > PATH_SIZE)
    {
        size = PATH_SIZE;
...
void Squid_Instance::update(Squid_Params *parameters)
{
    int status, i;
    char text[128];
...
int Squid_Instance::transmit(struct Squid_Data *Squid_Data)
{
    int i, status;
    char text[128];
    uint32_t msecs;
    ...
    if ((!system_data->OperatorLatitude) && (Squid_Data->base_valid))
    {

        system_data->OperatorLatitude = Squid_Data->base_latitude;
        system_data->OperatorLongitude = Squid_Data->base_longitude;
        system_data->OperatorAltitudeGeo = Squid_Data->base_alt_m;
        ...
        ((msecs - last_msecs) > 74))
    {
        ...
        case 36: // Every 300 ms.
            ...
            if (Squid_Data->satellites >= SATS_LEVEL_2)
            {

                location_data->Status = ODID_STATUS_UNDECLARED;
                location_data->Direction = (float)Squid_Data->heading;
                location_data->SpeedHorizontal = M_MPH_MS * (float)Squid_Data->speed;
                location_data->SpeedVertical = INV_SPEED_V;
                location_data->Latitude = Squid_Data->latitude_d;
                location_data->Longitude = Squid_Data->longitude_d;
                location_data->Height = Squid_Data->alt_agl_m;
                location_data->AltitudeGeo = Squid_Data->alt_msl_m;

                location_data->TimeStamp = (float)((Squid_Data->minutes * 60) + Squid_Data->seconds) +
                                           0.01 * (float)Squid_Data->csecs;
            }
            else
            {
    ...
    if ((msecs - last_wifi) >= beacon_interval)
    {
        ...
        { // IDs and locations.
            ...
            status = transmit_wifi(Squid_Data, 0);
...
int Squid_Instance::transmit_wifi(struct Squid_Data *Squid_Data, int prepacked)
{
...
{
    Serial.printf("In transmit_ble for %s\n", params.uas_operator);
